...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  requestPremiumReview: async () => {
    ...
    try {
      await (supabase.from("profiles") as any).upsert({ user_id: user.id, payment_status: "pending" });
    } catch {
  ...
  loadProfile: async (userId: string) => {
    ...
    try {
      const { data, error } = await (supabase.from("profiles") as any).select("payment_status").eq("user_id", userId).single();
  ...
  initializeProfile: async (userId: string, paymentStatus: PaymentStatus) => {
    if (!supabase) return;
    try {
      await (supabase.from("profiles") as any).upsert({ user_id: userId, payment_status: paymentStatus });
    } catch {
